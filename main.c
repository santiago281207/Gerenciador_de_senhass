    ...
    while (1) // Ciclo que mantem sempre a correr
    {
        ...
        case '2': // Gerar password forte
            do
            {
                ...
                {
                    printf("ERRO-Valor nao esta entre %d e %d: \n", MIN_COMP_PASS, MAX_COMP_PASS);
                }
            ...
            {   
                fprintf(log,"\n");
                fprintf(log,"%dº Password gerada: %s", criadas, generated_password);
                criadas++;
            }
            break;
...
void Generate_Random_Password(char passwd[], int comp) // Gerar password aleatoria
{
    ...
    int choosen_character_type = 0; 
    size_t pos_choosen = 0;
    int i = 0;
    size_t size_upper_letters = strlen(upper_letters);
    size_t size_lower_letters = strlen(lower_letters);
    size_t size_digits = strlen(digits);
    size_t size_special_characters = strlen(special);
    ...
    for (i = 0; i < comp; i++)
    {
        ...
        //printf("CHOOSEN CHARACTER TYPE - %d\n",choosen_character_type);
        if (choosen_character_type == 1)
        {
            pos_choosen = rand() % size_upper_letters;;
            passwd[i] = upper_letters[pos_choosen];
        }
        else if (choosen_character_type == 2)
        {
            pos_choosen = rand() % size_special_characters;;
            passwd[i] = special[pos_choosen];
        }
        else if (choosen_character_type == 3)
        {
            pos_choosen = rand() % size_digits;
            passwd[i] = digits[pos_choosen];
        }
        else if(choosen_character_type == 4)
        {
            pos_choosen = rand() % size_lower_letters;
